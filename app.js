  ...
  return h('div', { className: 'home-pad' }, [
    ...
    h('div', { style: { display: 'flex', flexDirection: 'column', gap: 14 } }, [
      ...trips.map(t => h('div', { key: t.id, className: 'trip-card', onClick: () => { setSid(t.id); setView('detail'); setSubTab('route') } }, [
        ...
        h('div', { className: 'trip-body', style: { position: 'relative' } }, [h('h3', null, t.name), h('div', { className: 'trip-dest', style: { display: 'flex', justifyContent: 'space-between' } }, [h('span', null, '📍 ' + (t.destination || '미정')), h('span', { style: { color: 'var(--blue)', fontWeight: 800 } }, getDDay(t.startDate))]), h('button', { className: 'icon-btn', style: { position: 'absolute', top: 0, right: 0, color: 'var(--red)' }, onClick: (e) => { e.stopPropagation(); if(confirm(`[${t.name}] 여행을 정말 삭제하시겠습니까?\n이 작업은 되돌릴 수 없습니다.`)) db.collection('trips').doc(t.id).delete().then(() => show('여행이 삭제되었습니다.')); } }, h(I, { n: 'delete', s: 18 }))])
      ])),
